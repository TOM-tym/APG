...
class iCIFAR10(DataHandler):
    ...
    test_transforms = [transforms.Resize(224),]
    common_transforms = [
        transforms.ToTensor(),
...
class ImageNetR(ImageNet100):
    imagenet_size = 200
    def base_dataset(self, data_path, train=True, download=False):
        logger.info(f'Using ImageNet-R dataset')

        split = "train" if train else "val"

        print("Loading metadata of ImageNetR_{} ({} split).".format(self.imagenet_size, split))
        metadata_path = os.path.join(
            data_path if self.metadata_path is None else self.metadata_path,
            "{}_imagenetR-{}{}.txt".format(split, self.imagenet_size, self.suffix)
        )
