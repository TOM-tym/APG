...
DevArray=(${DEVICES//,/ })
NumGpus=${#DevArray[@]}
LABEL=res_nonPretrained_CIFAR100_B50_T10
TODAY=$(date +"%Y%m%d")
TODAYDAY=${TODAY:6:8}
...
Config_stage0=options/continual_trans/non_pretrained/stage0/cifar100_stage0_deit_APG.yaml
Config_incremental=options/continual_trans/non_pretrained/imagenet_sub/cifar100_nonPretrained_incremental.yaml
...
CUDA_VISIBLE_DEVICES=$DEVICES python -m torch.distributed.launch --nproc_per_node=$NumGpus --master_port=$Master_port --use_env inclearn \
  ...
  --data-path ./datasets \
  --save task \
  --resume chkpts/my_deit_B50_85.5_no_cls/net_0_task_0.pth \
  --workers 4 \
  --force_from_stage0 \
  --only_stage0
